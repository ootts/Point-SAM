...
# class PatchEncoderOnnx(nn.Module):
#     def __init__(self, pc_encoder):
#         super(PatchEncoderOnnx, self).__init__()
#         self. = pc_encoder
#
...
def main():
    ...
    model = pc_encoder.patch_embed.patch_encoder

    patch_features = np.random.rand(1, 512, 64, 6)  # bsz,num_patches,patch_size,6
    patch_features=torch.from_numpy(patch_features).float().cuda()

    # Export torch model to ONNX
    output_onnx = args.output
    os.makedirs(os.path.dirname(output_onnx), exist_ok=True)
    print("Exporting ONNX model {}".format(output_onnx))
    torch.onnx.export(model, patch_features, output_onnx,
                      export_params=True,
                      verbose=False,
                      opset_version=17,
